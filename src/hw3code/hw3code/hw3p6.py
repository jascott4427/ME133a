...
class Trajectory():
    ...
    def evaluate(self, t, dt):
        ...
        omega_1  = ((2*pi)/3)*cos(2*t)

        theta_2 = (pi/3)*sin(t)-(pi/9)*cos(6*t)
        omega_2 = (pi/3)*cos(t)+(6*pi/9)*sin(6*t)

        theta_3  = (pi / 6) * sin(2* t)
        omega_3  = ((2*pi)/6)*cos(2*t)

        theta_4 = (pi/6)*sin(t)-(pi/9)*cos(6*t)
        omega_4 = (pi/6)*cos(t)+(6*pi/9)*sin(6*t)

        # Return the position and velocity as python lists.
        return ([theta_1, theta_2, theta_3, theta_4], [omega_1, omega_2, omega_3, omega_4])
